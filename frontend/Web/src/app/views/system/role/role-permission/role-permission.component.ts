...
export class RolePermissionComponent implements OnInit {
  ...
  roleId = 0;
  objectType = 2; // 1: GroupRole ; 2: Role
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.paramMap.subscribe((data: ParamMap) => {
      this.roleId = Number(data.get('id'));
    });
    this.tblMenuService.getLstMenuByObjectId(this.roleId, this.objectType).subscribe(rs => {
  ...
  initialize() {
    this.tblMenuService.findAllToJson(-1, this.roleId, true).subscribe(rs => {
  ...
  configPermission() {
    ...
    this.tblMenuService.configPermission(this.roleId, -1, JSON.stringify(arrMenu)).subscribe({
      next: data => {
